...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if($errors == 0){
	...
	$sql = "SELECT email_address, password FROM entity_admin_th1998726 WHERE email_address = '$email'";
  ...
  if($errors == 0){
	//Create connection
		...
		$sql = "SELECT admin_id, first_name FROM entity_admin_th1998726 WHERE email_address = '$email'";
		$result = $conn->query($sql);
		//check if there is a match
		...
		setcookie('admin_id', $admin, time()+(86400*2), "/");
...
<button id="back" onclick="window.location.href='index.php'">Admin Dashboard</button>
</div>
